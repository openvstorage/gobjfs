...
/**/
typedef ShmemQueueUPtr EdgeQueueUPtr;
typedef ShmemQueueUPtr ASDQueueUPtr;
  ...
  while (1) {
    ...
    if (ret == 0) {
      switch (reqMsg.opcode_) { 
        ...
        case ADD_EDGE_REQ : 
          {
            auto edgeQueue = gobjfs::make_unique<ShmemQueue<GatewayMsg>>(reqMsg.edgePid_);
...
{
  asdQueue = gobjfs::make_unique<ShmemQueue<GatewayMsg>>(100, maxQueueLen);
