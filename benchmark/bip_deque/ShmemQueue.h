...
class ShmemQueue
{
  ...
  struct Remover {
    ...
    Remover(int pid) {
      std::string name = "n" + std::to_string(pid);
      std::string mutexName = name + "m";
      std::string condName = name + "c";

      //segment_.destroy<BaseQueue>(name.c_str());
      bip::shared_memory_object::remove(name.c_str());
      bip::named_mutex::remove(mutexName.c_str());
      bip::named_condition::remove(condName.c_str());
    }
  };
...
typedef std::unique_ptr<ShmemQueue<gobjfs::rora::GatewayMsg>> ShmemQueueUPtr;
typedef std::shared_ptr<ShmemQueue<gobjfs::rora::GatewayMsg>> ShmemQueueSPtr;
