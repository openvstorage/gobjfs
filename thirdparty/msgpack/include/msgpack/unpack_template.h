...
msgpack_unpack_func(msgpack_unpack_object,
                    _data)(msgpack_unpack_struct(_context) * ctx) {
...
#define push_simple_value(func)                                                \
  if (msgpack_unpack_callback(func)(user, &obj) < 0) {                         \
    goto _failed;                                                              \
  }                                                                            \
  goto _push
#define push_fixed_value(func, arg)                                            \
  if (msgpack_unpack_callback(func)(user, arg, &obj) < 0) {                    \
    goto _failed;                                                              \
  }                                                                            \
  goto _push
#define push_variable_value(func, base, pos, len)                              \
  if (msgpack_unpack_callback(func)(user, (const char *)base,                  \
                                    (const char *)pos, len, &obj) < 0) {       \
    goto _failed;                                                              \
  }                                                                            \
  goto _push
...
#define start_container(func, count_, ct_)                                     \
  ...
  if (msgpack_unpack_callback(func)(user, count_, &stack[top].obj) < 0) {      \
    goto _failed;                                                              \
  }                                                                            \
  if ((count_) == 0) {                                                         \
    obj = stack[top].obj;                                                      \
  ...
  if(top >= stack_size) {                                                      \
      if(stack_size == MSGPACK_EMBED_STACK_SIZE) {                             \
          size_t csize = sizeof(msgpack_unpack_struct(_stack)) *               \
  MSGPACK_EMBED_STACK_SIZE;                                                    \
          size_t nsize = csize * 2;                                            \
          msgpack_unpack_struct(_stack)* tmp =                                 \
  (msgpack_unpack_struct(_stack)*)malloc(nsize);                               \
          if(tmp == NULL) { goto _failed; }                                    \
          memcpy(tmp, ctx->stack, csize);                                      \
          ctx->stack = stack = tmp;                                            \
          ctx->stack_size = stack_size = MSGPACK_EMBED_STACK_SIZE * 2;         \
      } else {                                                                 \
          size_t nsize = sizeof(msgpack_unpack_struct(_stack)) *               \
  ctx->stack_size * 2;                                                         \
          msgpack_unpack_struct(_stack)* tmp =                                 \
  (msgpack_unpack_struct(_stack)*)realloc(ctx->stack, nsize);                  \
          if(tmp == NULL) { goto _failed; }                                    \
          ctx->stack = stack = tmp;                                            \
...
// ok
        ...
        case MSGPACK_CS_STR_8:
          again_fixed_trail_if_zero(MSGPACK_ACS_STR_VALUE, *(uint8_t *)n,
                                    _str_zero);
        case MSGPACK_CS_BIN_8:
          again_fixed_trail_if_zero(MSGPACK_ACS_BIN_VALUE, *(uint8_t *)n,
                                    _bin_zero);
        case MSGPACK_CS_EXT_8:
          again_fixed_trail_if_zero(MSGPACK_ACS_EXT_VALUE, (*(uint8_t *)n) + 1,
                                    _ext_zero);
    ...
    _push:
      ...
      case MSGPACK_CT_MAP_VALUE:
        if (msgpack_unpack_callback(_map_item)(user, &c->obj, c->map_key, obj) <
            0) {
          goto _failed;
        }
        if (--c->count == 0) {
